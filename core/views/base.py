...
class BaseView(BaseModelViewSet):
    ...
    serializers
        create a dict of actions in keys and serializer in values,
    ...
    action_permissions
        create a dict of actions in keys and permission in values,
        or you can set an `default` serializer for other actions
        example:
            >>> action_permissions = {
            >>>     "default": IsAuthenticated,
            >>>     "created": [IsAuthenticated, HasPermFoo, HasPermBar], # example list value
            >>>     ...
            >>> }
            key choices is all ModelViewSet actions
